...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="/Volumes/T7/repos/slidastro/.codex/gsd-local-patches"
  fi
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `/Volumes/T7/repos/slidastro/.codex/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
