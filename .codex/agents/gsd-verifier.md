...
<required_reading>
@/Volumes/T7/repos/slidastro/.codex/get-shit-done/references/verification-overrides.md
...
@/Volumes/T7/repos/slidastro/.codex/get-shit-done/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@/Volumes/T7/repos/slidastro/.codex/get-shit-done/references/few-shot-examples/verifier.md
...
node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap analyze --raw)
```
