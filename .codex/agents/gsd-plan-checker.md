...
@/Volumes/T7/repos/slidastro/.codex/get-shit-done/references/few-shot-examples/plan-checker.md
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "/Volumes/T7/repos/slidastro/.codex/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
