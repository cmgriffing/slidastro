...
@/Volumes/T7/repos/slidastro/.gemini/get-shit-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
```bash
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
See `/Volumes/T7/repos/slidastro/.gemini/get-shit-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
```bash
node /Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs commit-to-subrepo "{type}({phase}-{plan}): {description}" --files file1 file2 ...
```
...
See /Volumes/T7/repos/slidastro/.gemini/get-shit-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `/Volumes/T7/repos/slidastro/.gemini/get-shit-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `/Volumes/T7/repos/slidastro/.gemini/get-shit-done/templates/summary.md`.
...
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" state record-metric \
...
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "/Volumes/T7/repos/slidastro/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
