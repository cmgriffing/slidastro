...
- **Global scope**: `/Volumes/T7/repos/slidastro/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Volumes/T7/repos/slidastro/.claude/notes/`)
...
2. Glob `/Volumes/T7/repos/slidastro/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Volumes/T7/repos/slidastro/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Volumes/T7/repos/slidastro/.claude/notes/` — works in any directory
