...
```bash
timestamp=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" commit "wip: [context-name] paused at [X]/[Y]" --files [handoff-path] .planning/HANDOFF.json
```
</step>
