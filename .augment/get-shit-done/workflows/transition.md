...
```bash
TRANSITION=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
if [ -n "$GSD_WORKSTREAM" ]; then
  WS_LIST=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" workstream list --raw)
fi
```
...
```bash
node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
