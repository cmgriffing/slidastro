...
```bash
PHASE_INFO=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
