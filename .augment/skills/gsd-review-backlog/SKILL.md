...
4. **For items to PROMOTE:**
   ...
   - Rename the directory from `999.x-slug` to `{new_num}-slug`:
     ```bash
     NEW_NUM=$(node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" phase add "${DESCRIPTION}" --raw)
     ```
...
6. **Commit changes:**
   ```bash
   node "/Volumes/T7/repos/slidastro/.augment/get-shit-done/bin/gsd-tools.cjs" commit "docs: review backlog — promoted N, removed M" --files .planning/ROADMAP.md
   ```
